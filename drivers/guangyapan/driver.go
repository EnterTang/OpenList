...
func (d *GuangYaPan) MakeDir(ctx context.Context, parentDir model.Obj, dirName string) (model.Obj, error) {
	...
	if name == "" {
		return nil, errors.New("dir name is empty")
	}
	...
	if !strings.EqualFold(strings.TrimSpace(out.Msg), "success") {
		return nil, fmt.Errorf("make dir failed: %s", strings.TrimSpace(out.Msg))
	}
	return &model.Object{
		ID:       out.Data.FileID,
		Name:     firstNonEmpty(out.Data.FileName, name),
		Size:     0,
		Modified: unixOrZero(out.Data.UTime),
		Ctime:    unixOrZero(out.Data.CTime),
		IsFolder: true,
	}, nil
}
...
func (d *GuangYaPan) Rename(ctx context.Context, srcObj model.Obj, newName string) (model.Obj, error) {
	...
	if !strings.EqualFold(strings.TrimSpace(out.Msg), "success") {
		return nil, fmt.Errorf("rename failed: %s", strings.TrimSpace(out.Msg))
	}
	return &model.Object{
		ID:       fileID,
		Name:     name,
...
func (d *GuangYaPan) Move(ctx context.Context, srcObj, dstDir model.Obj) (model.Obj, error) {
	if err := d.ensureAccessToken(ctx); err != nil {
		return nil, err
	}
	...
	if !strings.EqualFold(strings.TrimSpace(out.Msg), "success") {
		return nil, fmt.Errorf("move failed: %s", strings.TrimSpace(out.Msg))
	}
...
func (d *GuangYaPan) Copy(ctx context.Context, srcObj, dstDir model.Obj) (model.Obj, error) {
	if err := d.ensureAccessToken(ctx); err != nil {
		return nil, err
	}
	...
	if !strings.EqualFold(strings.TrimSpace(out.Msg), "success") {
		return nil, fmt.Errorf("copy failed: %s", strings.TrimSpace(out.Msg))
	}
	taskID := strings.TrimSpace(out.Data.TaskID)
	if taskID != "" {
		if err := d.waitTaskDone(ctx, taskID); err != nil {
	...
	// API does not return the new file id; let op synthesize cache entry.
	return nil, nil
}
...
func (d *GuangYaPan) Put(ctx context.Context, dstDir model.Obj, file model.FileStreamer, up driver.UpdateProgress) (model.Obj, error) {
	...
	if taskID == "" {
		return &model.Object{Name: name, Size: file.GetSize(), IsFolder: false}, nil
	}
	fileID, err := d.waitUploadTaskInfo(ctx, taskID)
	if err != nil {
		return nil, err
...
func (d *GuangYaPan) waitUploadTaskInfo(ctx context.Context, taskID string) (string, error) {
	const (
		maxTry   = 300
...
var _ driver.MoveResult = (*GuangYaPan)(nil)
var _ driver.RenameResult = (*GuangYaPan)(nil)
var _ driver.CopyResult = (*GuangYaPan)(nil)
