...
func (d *Yun139) CreateMobileShare(ctx context.Context, obj model.Obj, args model.MobileShareCreateArgs) (*model.MobileShareLink, error) {
	...
	plan, planErr := d.buildShareRiskRelocatePlan(ctx, obj, actualPath)
	if planErr != nil {
		return nil, fmt.Errorf("%w (auto relocate planning failed: %v)", err, planErr)
	}
	if plan == nil {
		return nil, err
	}
	newRoot, applyErr := d.applyShareRiskRelocatePlan(ctx, plan)
	if applyErr != nil {
		return nil, fmt.Errorf("%w (auto relocate apply failed: %v)", err, applyErr)
	}
	if syncErr := d.syncShareRiskRelocatePlan(ctx, plan, newRoot); syncErr != nil {
		return nil, fmt.Errorf("%w (auto relocate sync failed: %v)", err, syncErr)
	}
	retryArgs := args
	if strings.TrimSpace(retryArgs.SourcePath) != "" {
		retryArgs.SourcePath = plan.NewRootPath
	}
	retried, retryErr := d.createMobileShareOnce(ctx, newRoot, retryArgs)
	if retryErr != nil {
		return nil, fmt.Errorf("个人云未知异常，已尝试自动迁移后重新创建分享，但仍失败: %w", retryErr)
	}
	if retried != nil {
		retried.CanonicalTitle = plan.CanonicalTitle
		retried.SourcePath = plan.NewRootPath
		retried.SourceName = plan.NewRootName
	}
	return retried, nil
}
